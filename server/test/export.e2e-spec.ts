...
describe('Export (E2E) - view csv', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.user.create({ data: { username: 'admin_e2e_export', password: adminPwd, role: Role.admin } });
    const loginResp = await request(http).post('/api/auth/login').send({ username: 'admin_e2e_export', password: 'admin123' });
    ...
    const r1 = await request(http)
      ...
      .send({ values: { [fieldAId]: 3, [fieldBId]: 7, [fieldSumId]: null }, formulas: { [fieldSumId]: 'A + B' } });
    expect(r1.status).toBe(201);
    const r2 = await request(http)
      .post(`/api/tables/${tableId}/records`)
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ values: { [fieldAId]: 2, [fieldBId]: 8, [fieldSumId]: null }, formulas: { [fieldSumId]: 'A + B' } });
    ...
    const createView = await request(http)
      ...
      .send({
        name: 'V-Grid-Export',
        type: ViewType.grid,
        configJson: { sort: { fieldId: fieldAId, direction: 'asc' } },
  ...
  it('POST /api/views/:id/export returns CSV with headers', async () => {
    const resp = await request(http)
      .post(`/api/views/${viewId}/export`)
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ format: 'csv' });
