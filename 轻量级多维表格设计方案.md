...
## 20. 版本与仓库（v0.0.2）
- 版本号：根、`server`、`web` 的 `package.json` 统一为 `0.0.2`，作为系统发布版本。
- 远端：`origin=git@github.com:undustcover/sheets.git`（SSH）。
- 推送策略：优先使用 SSH，需在本机生成 `ed25519` 密钥并添加至 GitHub 账号后再执行 `git push origin main` 与 `git push origin v0.0.2`。
