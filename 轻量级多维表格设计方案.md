...
20. 测试与配置（2025-10-22）
- E2E 测试：已为 Records 模块补齐 E2E 覆盖（ACL/创建/列表/更新/公式/过滤排序）。
- 认证配置：在测试文件顶部显式设置 `JWT_SECRET` 与 `DATABASE_URL`，确保 `JwtModule` 与 `JwtStrategy` 初始化一致，避免 401。
- 登录状态码：`POST /api/auth/login` 返回 201（Nest 默认），测试应按 201 断言；Token 使用 `Bearer` 头访问 `/api/auth/me` 校验。
- 测试数据库：建议使用 `file:F:\trae\sheets\server\prisma\test-e2e.db` 并通过 `npx prisma db push --skip-generate` 预同步。
- 导出测试：视图与表级导出均通过 E2E；建议使用 `--runInBand` 串行运行，避免并发初始化导致的种子唯一约束冲突。
- 扩展计划：补充 Auth 的 `admin-check`、Cells 批量写入联动公式重算、Views 的过滤与排序的 E2E。
